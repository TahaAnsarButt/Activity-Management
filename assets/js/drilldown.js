				...
				if (this.drilldownLevels && 0 !== this.drilldownLevels.length) {
					for (
						...
						if (((c = b[g]), c.levelNumber === d)) {
							...
							c.resetZoomButton &&
								((a.resetZoomButton = c.resetZoomButton),
					...
					0 === this.drilldownLevels.length
						...
						: this.drillUpButton
								.attr({ text: this.getDrilldownBackText() })
								.align();
					this.ddDupes.length = [];
					y(a, "drillupall");
				}
					...
					O(function () {
						if (b.points) {
							...
							a.forEach(function (a, b) {
								...
								c &&
									!c.hidden &&
									(c.fadeIn(), a.connector && a.connector.fadeIn());
							});
						}
