...
(function (d) {
	...
	A(
		...
		function (d, e, l, a, c) {
			function t(b, m, n) {
				return (
					Math.sqrt(Math.pow(b - n[0], 2) + Math.pow(m - n[1], 2)) <= n[2] / 2
				);
			}
			var p = c.addEvent,
				k = c.extend,
			...
			c = (function () {
				...
				g.prototype.render = function () {
					...
					this.group ||
						(this.group = q
							.g("pane-group")
							.attr({ zIndex: g.zIndex || 0 })
							.add());
					this.updateCenter();
					if (n)
						for (
							n = b(n),
								g = Math.max(n.length, this.background.length || 0),
								q = 0;
							q < g;
							q++
						)
							n[q] && this.axis
								? this.renderBackground(
										x(this.defaultBackgroundOptions, n[q]),
										q
								  )
								: this.background[q] &&
								  ((this.background[q] = this.background[q].destroy()),
								  this.background.splice(q, 1));
				};
				g.prototype.renderBackground = function (b, g) {
					var n = "animate",
						m = { class: "highcharts-pane " + (b.className || "") };
					...
					this.background[g] ||
						((this.background[g] = this.chart.renderer.path().add(this.group)),
						(n = "attr"));
					this.background[g][n]({
			...
			p(d, "afterIsInsidePlot", function (b) {
				this.polar &&
					(b.isInsidePlot = this.pane.some(function (g) {
			...
			p(a, "beforeGetHoverData", function (b) {
				var g = this.chart;
				g.polar
					? ((g.hoverPane = g.getHoverPane(b)),
					  (b.filter = function (n) {
							return (
								n.visible &&
								!(!b.shared && n.directTouch) &&
								w(n.options.enableMouseTracking, !0) &&
								(!g.hoverPane || n.xAxis.pane === g.hoverPane)
							);
					  }))
					: (g.hoverPane = void 0);
			});
			p(a, "afterGetHoverData", function (b) {
				...
				b.hoverPoint &&
					...
					b.hoverPoint.plotY &&
					g.hoverPane &&
					!t(b.hoverPoint.plotX, b.hoverPoint.plotY, g.hoverPane.center) &&
					(b.hoverPoint = void 0);
			});
			l.Pane = c;
			return l.Pane;
		...
		function (d, e, l, a) {
			...
			(function (a) {
				function e() {
					this.autoConnect =
						this.isCircular &&
						"undefined" === typeof m(this.userMax, this.options.max) &&
						k(this.endAngleRad - this.startAngleRad) === k(2 * Math.PI);
				...
				function y() {
					...
					return function () {
						if (
							...
							h.tickPositions &&
							h.options.labels &&
							!0 !== h.options.labels.allowOverlap
						)
							return h.tickPositions
								.map(function (f) {
				...
				function f(h, f, b) {
					...
					if (this.isCircular) {
							...
							h.point &&
								...
								g.start &&
									(u = this.chart.inverted
										? this.translate(h.point.rectPlotY, !0)
										: h.point.x));
						else {
							var g = h.chartX || 0;
							var v = h.chartY || 0;
							u = this.translate(
								Math.atan2(v - b, g - f) - this.startAngleRad,
								!0
							);
						}
						...
						x(u) || ((g = h.chartX), (v = h.chartY)),
							x(g) &&
								x(v) &&
								((b = r[1] + this.chart.plotTop),
								(u = this.translate(
									Math.min(
										Math.sqrt(Math.pow(g - f, 2) + Math.pow(v - b, 2)),
										r[2] / 2
									) -
										r[3] / 2,
									!0
				...
				function h(h, f, b) {
					...
					"undefined" === typeof b &&
						(b = this.horiz ? 0 : this.center && -this.center[3] / 2);
					b && (v += b);
					this.isCircular || "undefined" !== typeof f
						? ((f = this.chart.renderer.symbols.arc(u + h[0], g + h[1], v, v, {
						  ...
						  (f.xBounds = [u + h[0]]),
						  (f.yBounds = [g + h[1] - v]))
						: ((f = this.postTranslate(this.angleRad, v)),
						  (f = [
				...
				function r(h, f, b) {
					...
					if ("polygon" === this.options.gridLineInterpolation)
						n = this.getPlotLinePath({ value: h }).concat(
						...
						z &&
							((z = (v + b) / 2),
							(q = q + g[0] + (g[2] / 2) * Math.cos(z)),
							(n.xBounds =
								z > -Math.PI / 2 && z < Math.PI / 2
									? [q, r.plotWidth]
									: [0, q]),
							(n.yBounds = [a + g[1] + (g[2] / 2) * Math.sin(z)]),
							(n.yBounds[0] +=
								(z > -Math.PI && 0 > z) || z > Math.PI ? -10 : 10));
					}
					return n;
				}
				function B(h) {
					var f = this,
						...
						u = r.inverted,
						g = h.reverse,
						v = this.pane.options.background
					...
					E = E.y;
					y &&
						((E = this.getCrosshairPosition(h, a, z)),
						(m = E[0]),
						(e = E[1]),
						(E = E[2]));
					if (this.isCircular)
						(m = Math.sqrt(Math.pow(e - a, 2) + Math.pow(E - z, 2))),
							(g = "string" === typeof B ? n(B, 1) : B / m),
							(r = "string" === typeof q ? n(q, 1) : q / m),
							b && v && ((v /= m), g < v && (g = v), r < v && (r = v)),
							(b = [
								["M", a + g * (e - a), z - g * (z - E)],
								["L", e - (1 - r) * (e - a), E + (1 - r) * (z - E)],
							]);
					else if (
						((m = this.translate(m)) && (0 > m || m > c) && (m = 0),
						"circle" === this.options.gridLineInterpolation)
					)
						b = this.getLinePath(0, m, v);
					else if (
						((b = []),
						r[u ? "yAxis" : "xAxis"].forEach(function (h) {
							h.pane === f.pane && (k = h);
						}),
						k)
					)
						for (
							...
							(v = k.getPosition(a[z], m)),
								b.push(z ? ["L", v.x, v.y] : ["M", v.x, v.y]);
					return b;
				}
				function v(h, f) {
					h = this.translate(h);
					return this.postTranslate(
						this.isCircular ? h : this.angleRad,
						m(this.isCircular ? f : 0 > h ? 0 : h, this.center[2] / 2) -
							this.offset
					);
				}
				function E() {
					...
					return {
						...
						y:
							f.plotTop +
							h[1] -
							{ high: 0.5, middle: 0.25, low: 0 }[b.align] * h[2] +
							(b.y || 0),
					};
				}
				function l(b) {
				...
				function L() {
					var h = this.chart,
						...
						b = this.pane,
						r = b && b.options;
					(h.angular && this.isXAxis) ||
						!b ||
						(!h.angular && !h.polar) ||
						((this.angleRad = ((f.angle || 0) * Math.PI) / 180),
						(this.startAngleRad = ((r.startAngle - 90) * Math.PI) / 180),
						(this.endAngleRad =
							((m(r.endAngle, r.startAngle + 360) - 90) * Math.PI) / 180),
					...
					if ("colorAxis" === B) this.isRadial = !1;
						...
						if (r) {
							if (
								(n
									? ((this.isHidden = !0),
									  (this.createLabelCollector = z),
									  (this.getOffset = t),
									  (this.render = this.redraw = G),
									  (this.setTitle = this.setCategories = this.setScale = t))
									: l(this),
								(q = !v))
							)
								this.defaultPolarOptions = R;
						} else
							u &&
								(l(this),
								(this.defaultPolarOptions = (q = this.horiz)
									? S
									: g(
											"xAxis" === B
					...
					if (f) {
						...
						if (b.isRadial) {
							var l = b.getPosition(
								this.pos,
								b.center[2] / 2 +
									n(m(u.distance, -25), b.center[2] / 2, -b.center[2] / 2)
							);
							"auto" === u.rotation
								? f.attr({ rotation: v })
								: x(a) ||
								  (a =
										b.chart.renderer.fontMetrics(f.styles && f.styles.fontSize)
											.b -
										r.height / 2);
							x(z) ||
								(b.isCircular
									? (r.width > (b.len * b.tickInterval) / (b.max - b.min) &&
											(q = 0),
									  (z =
											v > q && v < 180 - q
												? "left"
							...
							if (
								...
								180 < E && 360 >= E && (E = 360 - E);
								if (
									b.pane.options.startAngle === g ||
									b.pane.options.startAngle === g + 360 ||
									b.pane.options.startAngle === g - 360
								)
									c = "start";
								z =
								...
								15 > y || (180 <= y && 195 > y)
									? (k = 0.3 * r.height)
									: 15 <= y && 35 >= y
									? (k = "start" === c ? 0 : 0.75 * r.height)
									: 195 <= y && 215 >= y
									? (k = "start" === c ? 0.75 * r.height : 0)
									: 35 < y && 90 >= y
									? (k = "start" === c ? 0.25 * -r.height : r.height)
									: 215 < y &&
									  270 >= y &&
									  (k = "start" === c ? r.height : 0.25 * -r.height);
								15 > E
									? (e = "start" === c ? 0.15 * -r.height : 0.15 * r.height)
									: 165 < E &&
									  180 >= E &&
									  (e = "start" === c ? 0.15 * r.height : 0.15 * -r.height);
								f.attr({ align: z });
				...
				function W(h, f, b, r, u, g, v) {
					var B = this.axis;
					B.isRadial
						? ((h = B.getPosition(this.pos, B.center[2] / 2 + r)),
						  (f = ["M", f, b, "L", h.x, h.y]))
						: (f = h.call(this, f, b, r, u, g, v));
					return f;
				}
				var N = [],
					S = {
				...
				a.compose = function (h, f) {
					-1 === N.indexOf(h) &&
						(N.push(h),
						p(h, "afterInit", L),
						p(h, "autoLabelAlign", H),
						p(h, "destroy", K),
						p(h, "init", C),
						p(h, "initialAxisTranslation", U));
					-1 === N.indexOf(f) &&
						(N.push(f),
						p(f, "afterGetLabelPosition", J),
						p(f, "afterGetPosition", V),
						q(f.prototype, "getMarkPath", W));
					return h;
				};
			})(H || (H = {}));
			return H;
			...
			c = (function (a) {
				...
				c.prototype.translate = function () {
					...
					b.points.forEach(function (f) {
						...
						f.isNull
							? (f.plotY = null)
							: ((f.plotLow = u),
							  (f.plotHigh = g.translate(
									b.dataModify ? b.dataModify.modifyValue(h) : h,
									0,
									1,
									0,
									1
							  )),
							  b.dataModify && (f.yBottom = f.plotHigh));
					});
					this.chart.polar &&
						this.points.forEach(function (f) {
							b.highToXY(f);
							f.tooltipPos = [
								(f.plotHighX + f.plotLowX) / 2,
								(f.plotHigh + f.plotLow) / 2,
							];
						});
				};
				c.prototype.getGraphPath = function (b) {
					...
					for (h = b.length; h--; ) {
						var m = b[h];
						var k = a
							? { plotX: m.rectPlotX, plotY: m.yBottom, doCurve: !1 }
							: { plotX: m.plotX, plotY: m.plotY, doCurve: !1 };
					...
					c &&
						(!0 === c && (c = "left"),
						(r.step = { left: "right", center: "center", right: "left" }[c]));
					g = u.call(this, g);
					f = u.call(this, f);
					r.step = c;
					r = [].concat(b, g);
				...
				c.prototype.drawDataLabels = function () {
					...
					if (u) {
						...
						if (v.enabled || this._hasPointLabels) {
							for (f = a; f--; )
								if ((r = g[f])) {
									...
									r.below = q;
									B
										? v.align || (v.align = q ? "right" : "left")
										: v.verticalAlign ||
										  (v.verticalAlign = q ? "top" : "bottom");
								}
							this.options.dataLabels = v;
							b.drawDataLabels && b.drawDataLabels.apply(this, arguments);
							for (f = a; f--; )
								if ((r = g[f]))
									(r.dataLabelUpper = r.dataLabel),
										(r.dataLabel = h[f]),
										delete r.dataLabels,
										(r.y = r.low),
										(r.plotY = r._plotY);
							...
							for (f = a; f--; )
								if ((r = g[f]))
									(q = c.inside
										...
										(r.below = !q),
										B
											? c.align || (c.align = q ? "left" : "right")
											: c.verticalAlign ||
											  (c.verticalAlign = q ? "bottom" : "top");
						...
						if (v.enabled)
							for (f = a; f--; )
								if ((r = g[f]))
									r.dataLabels = [r.dataLabelUpper, r.dataLabel].filter(
										function (h) {
											return !!h;
										}
									);
				...
				c.prototype.drawPoints = function () {
					...
					for (c = 0; c < a; ) {
						...
						this.chart.polar ||
							(f.isInside = f.isTopInside =
								"undefined" !== typeof f.plotY &&
								0 <= f.plotY &&
								f.plotY <= this.yAxis.len &&
								0 <= f.plotX &&
								f.plotX <= this.xAxis.len);
						c++;
					}
					b.drawPoints.apply(this, arguments);
					for (c = 0; c < a; )
						(f = this.points[c]),
							(f.upperGraphic = f.graphic),
							(f.graphic = f.lowerGraphic),
							f.origProps && (m(f, f.origProps), delete f.origProps),
							c++;
				};
	...
	A(
		...
		function (d, e, l) {
			...
			l(p.prototype, { getPointSpline: c.prototype.getPointSpline });
				...
				e.prototype.drawPoints = function () {
					...
					b.points.forEach(function (n) {
						...
						"undefined" !== typeof n.plotY &&
							((h = Math.round(t.width)),
							(u = Math.floor(t.x)),
							(r = u + h),
							(B = Math.round(h / 2)),
							(q = Math.floor(v ? n.q1Plot : n.lowPlot)),
							(e = Math.floor(v ? n.q3Plot : n.lowPlot)),
							(l = Math.floor(n.highPlot)),
							(p = Math.floor(n.lowPlot)),
							m ||
								((n.graphic = m = c.g("point").add(b.group)),
								(n.stem = c.path().addClass("highcharts-boxplot-stem").add(m)),
								d &&
									(n.whiskers = c
										.path()
								...
								(n.medianShape = c
									...
									.add(m))),
							a.styledMode ||
								((I.stroke = n.stemColor || g.stemColor || M),
								(I["stroke-width"] = k(n.stemWidth, g.stemWidth, g.lineWidth)),
								(I.dashstyle =
									n.stemDashStyle || g.stemDashStyle || g.dashStyle),
								n.stem.attr(I),
								d &&
									((w.stroke = n.whiskerColor || g.whiskerColor || M),
									(w["stroke-width"] = k(
										n.whiskerWidth,
										g.whiskerWidth,
										g.lineWidth
									...
									(w.dashstyle =
										n.whiskerDashStyle || g.whiskerDashStyle || g.dashStyle),
									n.whiskers.attr(w)),
								v &&
									((x.fill = n.fillColor || g.fillColor || M),
									(x.stroke = g.lineColor || M),
									(x["stroke-width"] = g.lineWidth || 0),
									(x.dashstyle =
										n.boxDashStyle || g.boxDashStyle || g.dashStyle),
									n.box.attr(x)),
								(L.stroke = n.medianColor || g.medianColor || M),
								(L["stroke-width"] = k(
									n.medianWidth,
									g.medianWidth,
									g.lineWidth
								...
								(L.dashstyle =
									n.medianDashStyle || g.medianDashStyle || g.dashStyle),
								n.medianShape.attr(L)),
							(z = (n.stem.strokeWidth() % 2) / 2),
							(f = u + B + z),
							(m = [
								["M", f, e],
								["L", f, l],
							...
							v &&
								...
								n.box[H]({ d: m })),
							d &&
								((z = (n.whiskers.strokeWidth() % 2) / 2),
								(l += z),
								(p += z),
								(E = /%$/.test(d) ? (B * parseFloat(d)) / 100 : d / 2),
								(m = [
									["M", f - E, l],
									["L", f + E, l],
									["M", f - E, p],
								...
								n.whiskers[H]({ d: m })),
							(y = Math.round(n.medianPlot)),
							(z = (n.medianShape.strokeWidth() % 2) / 2),
							(y += z),
							(m = [
								["M", u, y],
								["L", r, y],
			...
			return (function () {
				...
				a.prototype.drawLegendSymbol = function (a) {
					...
					k && k.length && x(k[0].value)
						? (g(k, function (b, f) {
						  ...
						  (k = this.ranges[0].radius),
						  (c = 2 * k),
						  (m = m - k + a.width),
						  (m = 0 < m ? m : 0),
						  (this.maxLabel = a),
						  (this.movementX = "left" === n.labels.align ? m : 0),
						  (this.legendItemWidth = c + m + e),
				...
				a.prototype.setOptions = function () {
					var a = this.ranges,
						...
						m = { zIndex: g.zIndex, "stroke-width": g.borderWidth },
						l = { zIndex: g.zIndex, "stroke-width": g.connectorWidth },
						z = {
							align:
								this.legend.options.rtl || "left" === g.labels.align
									? "right"
									: "left",
							zIndex: g.zIndex,
						},
					...
					a.forEach(function (u, r) {
						h ||
							((m.stroke = b(u.borderColor, g.borderColor, k.color)),
							(m.fill = b(
								u.color,
								g.color,
								1 !== f ? c(k.color).setOpacity(f).get("rgba") : k.color
							)),
							(l.stroke = b(u.connectorColor, g.connectorColor, k.color)));
						a[r].radius = this.getRangeRadius(u.value);
						a[r] = w(a[r], { center: a[0].radius - a[r].radius + e });
						h ||
							w(!0, a[r], {
								bubbleAttribs: w(m),
				...
				a.prototype.renderRange = function (b) {
					...
					"center" === r &&
						...
						(b.labelAttribs.align = "center"));
					r = m + a.labels.y;
					var p = v + u + a.labels.x;
					e.bubbleItems.push(
						k
							.circle(v, f + ((m % 1 ? 1 : 0.5) - (B % 2 ? 0 : 0.5)), h)
							.attr(d ? {} : b.bubbleAttribs)
							.addClass(
								(d ? "highcharts-color-" + n.colorIndex + " " : "") +
					...
					b = k
						.text(this.formatLabel(b), p, r + l)
						.attr(d ? {} : b.labelAttribs)
						.css(d ? {} : g.style)
						.addClass(
							"highcharts-bubble-legend-labels " + (a.labels.className || "")
						)
						.add(this.legendSymbol);
					c.push(b);
					b.placed = !0;
					b.alignAttr = { x: p, y: r + l };
				};
				...
				a.prototype.hideOverlappingLabels = function () {
					...
					!this.options.labels.allowOverlap &&
						a &&
						(b.hideOverlappingLabels(a.labels),
						a.labels.forEach(function (b, g) {
							b.newOpacity
								? b.newOpacity !== b.oldOpacity && a.connectors[g].show()
								: a.connectors[g].hide();
						}));
				};
					...
					a.chart.series.forEach(function (a) {
						a.isBubble &&
							...
							c.length &&
								...
								(m = b(a.options.zMax, Math.max(m, p(c))))));
					});
					var l =
						e === m
							? [{ value: m }]
			...
			(function (a) {
				function b(b, a, c) {
					...
					if (
						...
						f.options.enabled &&
						f.bubbleLegend &&
						f.options.bubbleLegend.autoRanges &&
						h
					) {
				...
				function g(b) {
					...
					for (var a = 0; a < b.length; ) {
						if (b[a] && b[a].isBubble && b[a].visible && b[a].zData.length)
							return a;
						a++;
					}
					return -1;
				...
				function m(b) {
					...
					for (f = 0; f < g; f++)
						if (
							(b[f].legendItemHeight &&
								(b[f].itemHeight = b[f].legendItemHeight),
							b[f] === b[g - 1] ||
								(b[f + 1] &&
									b[f]._legendItemPos[1] !== b[f + 1]._legendItemPos[1]))
						) {
							a.push({ height: 0 });
							var u = a[a.length - 1];
							for (h; h <= f; h++)
				...
				function n(b) {
					...
					a &&
						a.ranges &&
						a.ranges.length &&
						(f.ranges.length && (f.autoRanges = !!f.ranges[0].autoRanges),
						this.destroyItem(a));
					0 <= h &&
						c.enabled &&
						f.enabled &&
						((f.seriesIndex = h),
						(this.bubbleLegend = new e(f, this)),
				...
				function l() {
					...
					c &&
						...
						(this.ignoreSeries = a),
						(b = 0 <= g(b)),
						c.bubbleLegend.visible !== b &&
							(c.update({ bubbleLegend: { enabled: b } }),
							(c.bubbleLegend.visible = b)),
						(this.visible = a));
				}
				function x(b, a) {
					...
					b.allItems.forEach(function (b, c) {
						...
						if ((u = b.movementX) || (g && b.ranges))
							(u = g ? f - b.options.maxSize / 2 : f + u),
								b.legendGroup.attr({ translateX: u });
						c > a[r].step && r++;
						b.legendGroup.attr({ translateY: Math.round(h + a[r].height / 2) });
		...
		function (d, e, l) {
			...
			e = (function (c) {
				...
				e.prototype.haloPath = function (a) {
					return d.prototype.haloPath.call(
						this,
		...
		function (d, e, l, a, c, t, p, k) {
			...
			k = (function (b) {
					...
					!b &&
						...
						this.points.forEach(function (b) {
							...
							h &&
								h.width &&
								(this.hasRendered ||
									h.attr({ x: b.plotX, y: b.plotY, width: 1, height: 1 }),
				...
				f.prototype.getRadii = function () {
					...
					if (!c) {
						...
						this.chart.series.forEach(function (h) {
							h.bubblePadding &&
								(h.visible || !b.chart.options.chart.ignoreHiddenSeries) &&
								(h = h.getZExtremes()) &&
								((n = Math.min(n || h.zMin, h.zMin)),
								(l = Math.max(l || h.zMax, h.zMax)),
								(d = !0));
						});
						d
							? ((c = { zMin: n, zMax: l }), (this.chart.bubbleZExtremes = c))
							: (c = { zMin: 0, zMax: 0 });
					}
					var p = 0;
				...
				f.prototype.translateBubble = function () {
					for (
						var b = this.data,
							f = this.radii,
							a = this.getPxExtremes().minPxSize,
							g = b.length;
						g--;
				...
				f.prototype.getPxExtremes = function () {
					...
					return { minPxSize: a, maxPxSize: f };
				};
				f.prototype.getZExtremes = function () {
					...
					if (f.length) {
						...
						b = y(b.zMax, m(f));
						if (K(a) && K(b)) return { zMin: a, zMax: b };
					}
				};
				f.compose = e.compose;
			...
			d.prototype.beforePadding = function () {
				...
				this.series.forEach(function (f) {
					if (
						f.bubblePadding &&
						(f.visible || !h.options.chart.ignoreHiddenSeries)
					) {
				...
				n &&
					...
					!this.logarithmic &&
					((g -= f),
					(k *= (f + Math.max(0, a) - Math.min(g, f)) / f),
					[
						["min", "userMin", a],
						["max", "userMax", g],
					].forEach(function (h) {
						"undefined" === typeof y(b.options[h[0]], b[h[1]]) &&
							(b[h[0]] += h[2] / k);
					}));
			};
	...
	A(
		...
		function (d, e) {
			...
			c(e.prototype, { setState: d.prototype.setState });
			return e;
		}
	);
	A(
		...
		function (d, e, l, a) {
			...
			t = (function (a) {
				...
				e.prototype.setOptions = function () {
					g(!0, arguments[0], { stacking: void 0 });
				...
				e.prototype.translate = function () {
					...
					a.points.forEach(function (v) {
						...
						u
							? ((f = v.barX + e),
							  (v.shapeType = "arc"),
							  (v.shapeArgs = a.polarArc(l + d, l, f, f + v.pointWidth)))
							: ((n.height = d),
							  (n.y = l),
							  (B = n.x),
							  (B = void 0 === B ? 0 : B),
							  (n = n.width),
							  (n = void 0 === n ? 0 : n),
							  (v.tooltipPos = h.inverted
									? [
											g.len + g.pos - h.plotLeft - l - d / 2,
											c.len + c.pos - h.plotTop - B - n / 2,
											d,
									  ]
									: [
											c.left - h.plotLeft + B + n / 2,
											g.pos - h.plotTop + l + d / 2,
											d,
									  ]));
					});
				};
				...
				l.prototype.translate = function () {
					...
					b.points.forEach(function (g) {
						...
						g.tooltipPos = a.inverted
							? [d.len + d.pos - a.plotLeft - v, b.xAxis.len - u - m, c]
							: [u + m, v + d.pos - a.plotTop, c];
						v = l + (g.total || g.y);
						"percent" === e.stacking && (v = l + (0 > g.y) ? -100 : 100);
						v = d.toPixels(v, !0);
						var w = (q = a.plotHeight - v - (a.plotHeight - p))
							? (m * (n - v)) / q
							: 0;
						var y = q ? (m * (n + c - v)) / q : 0;
						q = u - w + m;
			...
			a = (function (a) {
				...
				e.prototype.translate = function () {
					...
					this.points.forEach(function (d) {
						...
						if (x(v) || !1 === c.wrap)
							(v = x(v) ? (v / 180) * Math.PI : 0),
								(p = k(p, a.startAngleRad - v, a.endAngleRad + v));
						p = (180 * p) / Math.PI;
						d.shapeType = "path";
						d.shapeArgs = {
							d: n.path || [
				...
				e.prototype.drawPoints = function () {
					...
					a.points.forEach(function (f) {
						...
						g
							? (g.animate(e), (e.d = u))
							: (f.graphic = h[f.shapeType](e)
									.attr({ rotation: e.rotation, zIndex: 1 })
									.addClass("highcharts-dial")
									.add(a.group));
						if (!c.styledMode)
							f.graphic[g ? "animate" : "attr"]({
					...
					d
						? d.animate({ translateX: e[0], translateY: e[1] })
						: ((a.pivot = h
								.circle(0, 0, g(f.radius, 5))
								.attr({ zIndex: 2 })
				...
				e.prototype.animate = function (b) {
					...
					b ||
						a.points.forEach(function (b) {
							var g = b.graphic;
							g &&
								(g.attr({ rotation: (180 * a.yAxis.startAngleRad) / Math.PI }),
								g.animate(
				...
				l.prototype.select = function () {
					...
					this.isParentNode
						? ((a.getSelectedPoints = a.getSelectedParentNodes),
						  ...
						  (a.getSelectedPoints = d.prototype.getSelectedPoints))
						: e.prototype.select.apply(this, arguments);
				};
				return l;
		...
		function (d, e, l) {
			...
			e.dragNodesMixin = {
				...
				onMouseMove: function (a, e) {
					if (a.fixedPosition && a.inDragMode) {
						...
						if (5 < Math.abs(e) || 5 < Math.abs(d))
							(l = a.fixedPosition.plotX - e),
								(w = a.fixedPosition.plotY - d),
								c.isInsidePlot(l, w) &&
									((a.plotX = l),
									(a.plotY = w),
									(a.hasDragged = !0),
			...
			a(d, "load", function () {
				var c = this,
					...
					l;
				c.container &&
					(e = a(c.container, "mousedown", function (e) {
						var k = c.hoverPoint;
						k &&
							k.series &&
							k.series.hasDraggableNodes &&
							k.series.options.draggable &&
							(k.series.onMouseDown(k, e),
							(d = a(c.container, "mousemove", function (b) {
				...
				verlet: {
					...
					barycenter: function () {
						...
						this.nodes.forEach(function (c) {
							c.fixedPosition ||
								((c.plotX -= d / c.mass / c.degree),
								(c.plotY -= a / c.mass / c.degree));
						});
					},
					...
					attractive: function (e, d, a) {
						...
						e.fromNode.fixedPosition ||
							((e.fromNode.plotX -= (l * c.fromNode) / e.fromNode.degree),
							(e.fromNode.plotY -= (d * c.fromNode) / e.fromNode.degree));
						e.toNode.fixedPosition ||
							((e.toNode.plotX += (l * c.toNode) / e.toNode.degree),
							(e.toNode.plotY += (d * c.toNode) / e.toNode.degree));
					},
					integrate: function (e, d) {
						...
						d.plotX += l;
						d.plotY += a;
						d.temperature = e.vectorLength({ x: l, y: a });
					},
					getK: function (e) {
						return Math.pow((e.box.width * e.box.height) / e.nodes.length, 0.5);
					},
				},
				euler: {
					attractiveForceFunction: function (e, d) {
					...
					attractive: function (e, d, a, c) {
						...
						d *= a.y / c;
						e.fromNode.fixedPosition ||
							((e.fromNode.dispX -= (p * l.fromNode) / e.fromNode.degree),
							(e.fromNode.dispY -= (d * l.fromNode) / e.fromNode.degree));
						e.toNode.fixedPosition ||
							((e.toNode.dispX += (p * l.toNode) / e.toNode.degree),
							(e.toNode.dispY += (d * l.toNode) / e.toNode.degree));
					},
					integrate: function (e, d) {
						...
						0 !== a &&
							((d.plotX +=
								(d.dispX / a) * Math.min(Math.abs(d.dispX), e.temperature)),
							(d.plotY +=
								(d.dispY / a) * Math.min(Math.abs(d.dispY), e.temperature)));
					},
					getK: function (e) {
						return Math.pow((e.box.width * e.box.height) / e.nodes.length, 0.3);
					},
				},
			};
		}
	...
	A(
		...
		function (d, e) {
			...
			e(l.prototype, {
				insert: function (a, c) {
					this.isInternal
						? this.nodes[this.getBoxPosition(a)].insert(a, c - 1)
						: ((this.isEmpty = !1),
						  this.body
								? c
									? ((this.isInternal = !0),
			...
			d = d.QuadTree = function (a, c, e, d) {
				this.box = { left: a, top: c, width: e, height: d };
				this.maxDepth = 25;
	...
	A(
		...
		function (d, e, l, a) {
			...
			var t = a.clamp,
				...
				x = a.isFunction,
				w = a.pick;
			l.layouts = { "reingold-fruchterman": function () {} };
			k(l.layouts["reingold-fruchterman"].prototype, {
				init: function (b) {
					...
					this.attractiveForce = w(
						b.attractiveForce,
						this.integration.attractiveForceFunction
					);
					this.repulsiveForce = w(
						b.repulsiveForce,
				...
				step: function () {
					...
					b.enableSimulation &&
						...
						b.maxIterations-- && isFinite(b.temperature) && !b.isStable()
							? (b.simulation && l.win.cancelAnimationFrame(b.simulation),
							  (b.simulation = l.win.requestAnimationFrame(function () {
									b.step();
							  })))
							: (b.simulation = !1));
				},
				...
				barnesHutApproximation: function (b, a) {
					...
					if (b !== a && 0 !== g)
						if (a.isInternal)
							...
							(e = this.repulsiveForce(g, this.k)),
								this.force("repulsive", b, e * a.mass, c, g);
					return d;
				},
				...
				attractiveForces: function () {
					...
					b.links.forEach(function (g) {
						g.fromNode &&
							g.toNode &&
							((a = b.getDistXY(g.fromNode, g.toNode)),
							(c = b.vectorLength(a)),
							0 !== c &&
								((e = b.attractiveForce(c, b.k)),
								b.force("attractive", g, e, a, c)));
					});
				},
				applyLimits: function () {
					...
					b.nodes.forEach(function (a) {
						a.fixedPosition ||
							(b.integration.integrate(b, a),
							b.applyLimitBox(a, b.box),
							(a.dispX = 0),
							(a.dispY = 0));
					});
				},
			...
			e(d, "predraw", function () {
				this.graphLayoutsLookup &&
					this.graphLayoutsLookup.forEach(function (b) {
						b.stop();
					});
			});
			e(d, "render", function () {
				function b(b) {
					b.maxIterations-- &&
						isFinite(b.temperature) &&
						!b.isStable() &&
						!b.enableSimulation &&
						(b.beforeStep && b.beforeStep(), b.step(), (e = !1), (a = !0));
				}
				var a = !1;
	...
	A(
		d,
		"Series/PackedBubble/PackedBubbleComposition.js",
		[d["Core/Chart/Chart.js"], d["Core/Globals.js"], d["Core/Utilities.js"]],
		function (d, e, l) {
			...
			e.networkgraphIntegrations.packedbubble = {
				...
				barycenter: function () {
					...
					b.forEach(function (g) {
						a.options.splitSeries && !g.isParentNode
							? ((d = g.series.parentNode.plotX),
							  (m = g.series.parentNode.plotY))
							: ((d = e.width / 2), (m = e.height / 2));
						g.fixedPosition ||
							((g.plotX -=
								((g.plotX - d) * c) / (g.mass * Math.sqrt(b.length))),
							(g.plotY -=
								((g.plotY - m) * c) / (g.mass * Math.sqrt(b.length))));
					});
				},
			...
			e.layouts.packedbubble = t(a, {
				...
				setCircularPositions: function () {
					...
					e.forEach(function (e, k) {
						a.options.splitSeries && !e.isParentNode
							? ((g = e.series.parentNode.plotX),
							  (d = e.series.parentNode.plotY))
							: ((g = c.width / 2), (d = c.height / 2));
				...
				repulsiveForces: function () {
					...
					a.nodes.forEach(function (d) {
						...
						a.nodes.forEach(function (k) {
							...
							d === k ||
								d.fixedPosition ||
								(!a.options.seriesInteraction && d.series !== k.series) ||
								((b = a.getDistXY(d, k)),
								(e =
									a.vectorLength(b) - (d.marker.radius + k.marker.radius + g)),
								0 > e &&
									((d.degree += 0.01),
									d.neighbours++,
									(c = a.repulsiveForce(
								...
								a.force("repulsive", d, c * k.mass, b, k, e));
						});
					});
				},
				applyLimitBox: function (c) {
					if (
						...
						var d =
							c.series.parentNodeRadius -
							c.marker.radius -
							this.vectorLength(e);
						0 > d &&
							d > -2 * c.marker.radius &&
							((c.plotX -= 0.01 * e.x), (c.plotY -= 0.01 * e.y));
					}
			...
			d = (function (a) {
				...
				f.prototype.accumulateAllPoints = function (a) {
					...
					for (f = 0; f < b.series.length; f++)
						if (
							((a = b.series[f]),
							(a.is("packedbubble") && a.visible) ||
								!b.options.chart.ignoreHiddenSeries)
						)
							for (c = 0; c < a.yData.length; c++)
								h.push([
									null,
									null,
									a.yData[c],
				...
				f.prototype.addLayout = function () {
					...
					b ||
						((this.chart.graphLayoutsStorage = b = {}),
						(this.chart.graphLayoutsLookup = f = []));
					var d = b[a.type];
					d ||
						((a.enableSimulation = g(c.forExport)
							? !c.forExport
							: a.enableSimulation),
						(b[a.type] = d = new e.layouts[a.type]()),
						d.init(a),
						f.splice(d.index, 0, d));
					this.layout = d;
					this.points.forEach(function (a) {
						a.mass = 2;
						a.degree = 1;
				...
				f.prototype.addSeriesLayout = function () {
					...
					d ||
						((b[a.type + "-series"] = d = new e.layouts[a.type]()),
						d.init(c),
						f.splice(d.index, 0, d));
				...
				f.prototype.checkOverlap = function (a, b) {
					var h = a[0] - b[0],
						f = a[1] - b[1];
					return -0.001 > Math.sqrt(h * h + f * f) - Math.abs(a[2] + b[2]);
				};
				f.prototype.createParentNodes = function () {
					var a = this,
						...
						k = {
							...
							lineColor: a.color,
							fillColor: p(a.color).brighten(0.4).get(),
						};
					...
					c ||
						(e ||
							(e = new d().init(this, {
								...
								marker: k,
								dataLabels: { inside: !1 },
								states: { normal: { marker: k }, hover: { marker: k } },
								dataLabelOnNull: !0,
						...
						a.parentNode &&
							((e.plotX = a.parentNode.plotX), (e.plotY = a.parentNode.plotY)),
						(a.parentNode = e),
						f.addElementsToCollection([a], f.series),
						f.addElementsToCollection([e], f.nodes));
				};
				...
				f.prototype.drawGraph = function () {
					if (this.layout && this.layout.options.splitSeries) {
						...
						var f = {
							fill: b.fillColor || p(this.color).brighten(0.4).get(),
						...
						this.parentNode.graphic ||
							(this.graph = this.parentNode.graphic =
								a.renderer.symbol(f.symbol).add(this.parentNodesGroup));
						...
						c &&
							...
							c.nodes.forEach(function (c) {
								a &&
									...
									((b = h.getDistXY(a, c)),
									(f = h.vectorLength(b) - c.marker.radius - a.marker.radius),
									0 > f &&
										(c.series.addPoint(
											A(a.options, { plotX: a.plotX, plotY: a.plotY }),
											!1
										),
										h.removeElementFromCollection(a, h.nodes),
										a.remove()));
							});
					...
					if (a.length) {
						...
						if (1 < a.length)
							for (
								...
								(a[k][2] = a[k][2] || 1),
									(g = f(c[h][e], c[h - 1][d], a[k])),
									b(g, c[h][0])
										? (c.push([]),
										  (d = 0),
										  c[h + 1].push(f(c[h][e], c[h][0], a[k])),
										  h++,
										  (e = 0))
										: 1 < h && c[h - 1][d + 1] && b(g, c[h - 1][d + 1])
										? (d++, c[h].push(f(c[h][e], c[h - 1][d], a[k])), e++)
										: (e++, c[h].push(g));
						this.chart.stages = c;
				...
				f.prototype.pointAttribs = function (a, b) {
					...
					a &&
						a.isParentNode &&
						f.layoutAlgorithm &&
						f.layoutAlgorithm.parentNodeOptions &&
						(c = f.layoutAlgorithm.parentNodeOptions.marker);
					f = c.fillOpacity;
				...
				f.prototype.positionBubble = function (a, b, f) {
					...
					a =
						(0 > a[1] - b[1] ? 0 : Math.PI) +
						e +
						h * (0 > (a[0] - b[0]) * (a[1] - b[1]) ? 1 : -1);
					return [
						b[0] + (b[2] + f[2]) * Math.sin(a),
						b[1] - (b[2] + f[2]) * Math.cos(a),
						f[2],
						f[3],
						f[4],
					];
				...
				f.prototype.resizeRadius = function () {
					...
					if (1e-10 < Math.abs(f - 1)) {
						...
						(a.diffY = g / 2 + d - l - (n - l) / 2),
							(a.diffX = k / 2 + e - p - (z - p) / 2);
				};
				...
				f.prototype.translate = function () {
					...
					this.generatePoints();
					g(a.allDataPoints) ||
						((a.allDataPoints = this.accumulateAllPoints(this)),
						this.getPointRadius());
					...
					for (c = 0; c < d.length; c++)
						if (d[c][3] === f) {
							...
							e ||
								((k.plotX = d[c][0] - a.plotLeft + a.diffX),
								(k.plotY = d[c][1] - a.plotTop + a.diffY));
							H(l) &&
								((k.marker = m(k.marker, {
									radius: l,
									width: 2 * l,
		...
		function (d, e, l, a) {
			...
			a = (function (a) {
				...
				e.prototype.getGraphPath = function () {
					for (
						var a = x.prototype.getGraphPath.call(this), b = a.length + 1;
						b--;
	...
	A(
		...
		[d["Extensions/Stacking.js"], d["Core/Utilities.js"]],
		function (d, e) {
			var l = e.addEvent,
				a = e.objectEach,
				c;
			...
			a = (function (a) {
							...
							if (a) {
								if (l) {
									...
									if ("overlap" === a) {
										...
										y = 0 <= D ? y : y - D;
										Object.hasOwnProperty.call(t, "absolutePos") &&
											delete t.absolutePos;
										Object.hasOwnProperty.call(t, "absoluteNeg") &&
											delete t.absoluteNeg;
									} else
										0 <= D
											? ((y = t.threshold + t.posTotal), (t.posTotal -= D))
											: ((y = t.threshold + t.negTotal),
											  (t.negTotal -= D),
											  (y -= D)),
											!t.posTotal &&
												Object.hasOwnProperty.call(t, "absolutePos") &&
												((t.posTotal = t.absolutePos), delete t.absolutePos),
											!t.negTotal &&
												Object.hasOwnProperty.call(t, "absoluteNeg") &&
												((t.negTotal = t.absoluteNeg), delete t.absoluteNeg);
									q.isSum ||
										(t.connectorThreshold = t.threshold + t.stackTotal);
									b.reversed
										? ((w = 0 <= D ? y - D : y + D), (D = y))
										: ((w = y), (D = y - D));
									q.below = w <= d;
									...
									C.height = Math.abs(
										C.y - (b.translate(D, !1, !0, !1, !0) || 0)
									);
									if ((D = b.waterfall.dummyStackItem))
										(D.x = p),
											(D.label = l[p].label),
											D.setOffset(
												this.pointXOffset || 0,
								...
								(y = Math.max(g, g + D) + t[0]),
									(C.y = b.translate(y, !1, !0, !1, !0) || 0),
									q.isSum
										? ((C.y = b.translate(t[1], !1, !0, !1, !0) || 0),
										  (C.height =
												Math.min(
													b.translate(t[0], !1, !0, !1, !0) || 0,
													b.len
												) - C.y),
										  (q.below = t[1] <= d))
										: q.isIntermediateSum
										? (0 <= D
												? ((w = t[1] + k), (D = k))
												: ((w = k), (D = t[1] + k)),
										  b.reversed && ((w ^= D), (D ^= w), (w ^= D)),
										  (C.y = b.translate(w, !1, !0, !1, !0) || 0),
										  (C.height = Math.abs(
												C.y -
													Math.min(b.translate(D, !1, !0, !1, !0) || 0, b.len)
										  )),
										  (k += t[1]),
										  (q.below = w <= d))
										: ((C.height =
												0 < w
													? (b.translate(g, !1, !0, !1, !0) || 0) - C.y
													: (b.translate(g, !1, !0, !1, !0) || 0) -
													  (b.translate(g - w, !1, !0, !1, !0) || 0)),
										  (g += w),
										  (q.below = g < d)),
									0 > C.height && ((C.y += C.height), (C.height *= -1));
							q.plotY = C.y = Math.round(C.y || 0) - (this.borderWidth % 2) / 2;
							C.height = Math.max(Math.round(C.height || 0), 0.001);
							q.yBottom = C.y + C.height;
							C.height <= c && !q.isNull
								? ((C.height = c),
								  (C.y -= e),
								  (q.plotY = C.y),
								  (q.minPointLengthOffset = 0 > q.y ? -e : e))
								: (q.isNull && (C.width = 0), (q.minPointLengthOffset = 0));
							D = q.plotY + (q.negative ? C.height : 0);
							q.below && (q.plotY += C.height);
							q.tooltipPos &&
								(this.chart.inverted
									? (q.tooltipPos[0] = b.len - D)
									: (q.tooltipPos[1] = D));
						}
					}
				};
				c.prototype.processData = function (b) {
					...
					for (t = n = l = p = q = 0; t < g; t++) {
						...
						"sum" === z || w.isSum
							? (e[t] = m(n))
							: "intermediateSum" === z || w.isIntermediateSum
							? ((e[t] = m(l)), (l = 0))
							: ((n += z), (l += z));
						p = Math.min(n, p);
						q = Math.max(n, q);
				...
				c.prototype.getCrispPath = function () {
					...
					for (n = 1; n < c; n++) {
						...
						w &&
							...
							((w = w[n - 1]),
							l
								? ((w = w.connectorThreshold),
								  (x =
										Math.round(b.translate(w, 0, 1, 0, 1) + (k ? x : 0)) - e))
								: (x = t.y + q.minPointLengthOffset + d - e),
							m.push(
								["M", (t.x || 0) + (g ? 0 : t.width || 0), x],
								["L", (p.x || 0) + (g ? p.width || 0 : 0), x]
							));
						t &&
							m.length &&
							((!l && 0 > q.y && !k) || (0 < q.y && k)) &&
				...
				c.prototype.setStackedPoints = function () {
					function a(a, b, c, f) {
						if (J) for (c; c < J; c++) A.stackState[c] += f;
				...
				c.prototype.getExtremes = function () {
					...
					if (a) {
						...
						return { dataMin: g(e), dataMax: b(d) };
					}
		...
		function (d, e, l, a, c, t, p, k, x) {
			...
			I.searchPointByAngle = function (a) {
				...
				return this.searchKDTree({
					clientX:
						...
						(-180 / Math.PI) *
							Math.atan2(
								a.chartX - c[0] - b.plotLeft,
								a.chartY - c[1] - b.plotTop
							),
				});
			};
			...
			I.toXY = function (a) {
				...
				k &&
					g &&
					!g.isRadialBar &&
					(a.plotY = e = "number" === typeof l ? f.translate(l) || 0 : 0);
				a.rectPlotX = d;
				a.rectPlotY = e;
				f.center && (m += f.center[3] / 2);
				n(e) &&
					((f = k ? f.postTranslate(e, m) : c.postTranslate(d, m)),
					(a.plotX = a.polarPlotX = f.x - b.plotLeft),
					(a.plotY = a.polarPlotY = f.y - b.plotTop));
				this.kdByAngle
					? ((b = ((d / Math.PI) * 180 + c.pane.options.startAngle) % 360),
					  0 > b && (b += 360),
					  (a.clientX = b))
					: (a.clientX = a.plotX);
			};
			p.spline &&
				(d(p.spline.prototype, "getPointSpline", function (a, b, c, d) {
					this.chart.polar
						? d
							? ((a = this.getConnectors(b, d, !0, this.connectEnds)),
							  ...
							  (a = [
									...
									n(b) ? b : a.plotX,
									n(c) ? c : a.plotY,
									n(a.leftContX) ? a.leftContX : a.plotX,
									n(a.leftContY) ? a.leftContY : a.plotY,
									a.plotX,
									a.plotY,
							  ]))
							: (a = ["M", c.plotX, c.plotY])
				...
				p.areasplinerange &&
					(p.areasplinerange.prototype.getPointSpline =
						p.spline.prototype.getPointSpline));
			b(
				t,
				"afterTranslate",
				function () {
								...
								b(this, "afterRender", function () {
									if (a.polar) {
										...
										this.clipCircle
											...
											: (this.clipCircle = a.renderer.clipCircle(
													b[0],
													b[1],
													b[2] / 2,
													b[3] / 2
			...
			var y = function (a, b) {
				...
				if (d.polar)
					if (c.isRadialBar)
						b ||
							((c.startAngleRad = q(
								c.translatedThreshold,
								c.xAxis.startAngleRad
							)),
							l.seriesTypes.pie.prototype.animate.call(c, b));
					else {
						if (d.renderer.isSVG)
								...
								if (!b) {
									...
									c.points.forEach(function (a) {
										...
										m &&
											p &&
											(m.attr({ r: y, innerR: y }),
											m.animate({ r: t, innerR: x }, c.options.animation));
									});
								}
							} else
								b
						...
						for (; e--; ) {
							...
							if (d.inverted) {
								...
								if (c && f.stacking) {
									if (
										((A =
											f.stacking.stacks[(0 > A ? "-" : "") + this.stackKey]),
										this.visible && A && A[z] && !b.isNull)
									) {
										var G =
											A[z].points[
												this.getStackIndicator(void 0, z, this.index).key
											];
								...
								b.plotY =
									(g(this.translatedThreshold) &&
										(F < this.translatedThreshold ? F : G)) - m;
							} else
								(F = y + m),
									(b.shapeArgs = this.polarArc(
										b.yBottom,
										b.plotY,
										F,
										F + b.pointWidth
							...
							this.toXY(b);
							d.inverted
								? ((y = f.postTranslate(b.rectPlotY, y + b.pointWidth / 2)),
								  (b.tooltipPos = [y.x - d.plotLeft, y.y - d.plotTop]))
						...
						? ((a = (b.rectPlotX / Math.PI) * 180),
						  f.inverted
								? ((this.forceDL = f.isInsidePlot(
								  ...
								  (d.align = q(d.align, "center")),
								  (d.verticalAlign = q(d.verticalAlign, "middle")))
								: this.findAlignments && (d = this.findAlignments(a, d)),
						  I.alignDataLabel.call(this, b, c, d, e, g),
						  this.isRadialBar &&
								b.shapeArgs &&
			...
			k.prototype.clipCircle = function (a, b, c, d) {
				...
				a = d
					? this.arc(a, b, c, d, 0, 2 * Math.PI).add(f)
					: this.circle(a, b, c).add(f);
				a.id = e;
				a.clipPath = f;
				return a;
			};
