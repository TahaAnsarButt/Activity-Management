...
(function (b) {
	"object" === typeof module && module.exports
		...
		? define("highcharts/modules/data", ["highcharts"], function (p) {
				b(p);
				b.Highcharts = p;
				return b;
		  })
				...
				w = {
					ajax: function (b) {
						...
						l.headers["Content-Type"] ||
							e.setRequestHeader("Content-Type", b[l.dataType] || b.text);
						D(l.headers, function (b, l) {
					...
					post: function (b, e, w) {
						...
						D(e, function (b, e) {
							p("input", { type: "hidden", name: e, value: b }, null, l);
						});
						l.submit();
						u(l);
					},
		...
		function (b, e, p, H, I, q) {
			...
			q = (function () {
				...
				b.prototype.init = function (a, c, h) {
					...
					this.firstRowAsNames = F(a.firstRowAsNames, this.firstRowAsNames, !0);
				...
				b.prototype.getColumnDistribution = function () {
					...
					c.forEach(function (c) {
						var h = new G(),
							t = b[k] || d(g),
							e = ((a && a.series) || [])[k] || {},
							C = v[e.type || g || "line"].prototype.pointArrayMap,
							p = C || ["y"];
						(l(c.x) || e.isCartesian || !C) && h.addColumnReader(c.x, "x");
						K(c, function (a, c) {
							"x" !== c && h.addColumnReader(a, c);
						});
						for (f = 0; f < t; f++)
							h.hasReader(p[f]) || h.addColumnReader(void 0, p[f]);
						m.push(h);
						k++;
					});
				...
				b.prototype.parseCSV = function (a) {
					function c(a, c, h, d) {
						...
						function g(a) {
							l.length < x + 1 && l.push([a]);
							l[x][l[x].length - 1] !== a && l[x].push(a);
						}
						function e() {
							f > r || r > C
								? (++r, (m = ""))
								: (!isNaN(parseFloat(m)) && isFinite(m)
										? ((m = parseFloat(m)), g("number"))
										: isNaN(Date.parse(m))
										? g("string")
										: ((m = m.replace(/\//g, "-")), g("date")),
								  p.length < x + 1 && p.push([]),
								  h || (p[x][c] = m),
								  (m = ""),
								  ++x,
						...
						a.some(function (a, d) {
							...
							for (var k = 0; k < a.length; k++) {
								...
								if ('"' === d)
									...
									"undefined" !== typeof r[d]
										? ((g = g.trim()),
										  isNaN(Date.parse(g))
												? (!isNaN(g) && isFinite(g)) || r[d]++
												: r[d]++,
										  (g = ""))
										: (g += d);
								"," === d && h++;
							...
							for (f = 0; f < e.length; f++)
								!1 !== e[f]
									? 12 < t[f] &&
									  ...
									  "YYYY" !== d[f] &&
									  (d[f] = "YY")
									: 12 < t[f] && "mm" === d[f] && (d[f] = "dd");
							3 === d.length && "dd" === d[1] && "dd" === d[2] && (d[2] = "YY");
							a = d.join("/");
							return (b.dateFormats || g.dateFormats)[a]
								? a
								: (E("deduceDateFailed"), "YYYY/mm/dd");
						}
						return "YYYY/mm/dd";
					}
					var g = this,
					...
					var k = b.endRow || Number.MAX_VALUE,
						f =
							"undefined" !== typeof b.startColumn && b.startColumn
								? b.startColumn
								: 0,
					...
					if (e) {
						...
						if (b.itemDelimiter) var q = b.itemDelimiter;
						else (q = null), (q = h(e));
						var z = 0;
						for (n = a; n <= k; n++) "#" === e[n][0] ? z++ : c(e[n], n - a - z);
						(b.columnTypes && 0 !== b.columnTypes.length) ||
							!l.length ||
							!l[0].length ||
							"date" !== l[0][1] ||
							b.dateFormat ||
				...
				b.prototype.parseTable = function () {
					...
					c &&
						("string" === typeof c && (c = u.getElementById(c)),
						[].forEach.call(c.getElementsByTagName("tr"), function (a, c) {
							c >= d &&
								c <= b &&
								[].forEach.call(a.children, function (a, b) {
									var g = h[b - e],
										f = 1;
									if (
										...
										for (
											h[b - e] || (h[b - e] = []),
												h[b - e][c - d] = a.innerHTML;
											c - d >= f && void 0 === g[c - d - f];
					...
					function a(g) {
						function f(f, k, t) {
							function n() {
								e &&
									h.liveDataURL === f &&
									(c.liveDataTimeout = setTimeout(a, m));
							}
							if (!f || !/^(http|\/|\.\/|\.\.\/)/.test(f))
								return f && d.error && d.error("Invalid URL"), !1;
							g && (clearTimeout(c.liveDataTimeout), (h.liveDataURL = f));
							w({
								url: f,
								dataType: t || "json",
								success: function (a) {
						...
						f(
							...
							f(k.rowsURL, function (a) {
								h.update({ data: { rows: a } });
							}) ||
							f(k.columnsURL, function (a) {
								h.update({ data: { columns: a } });
							});
					}
					var c = this,
						h = this.chart,
				...
				b.prototype.parseGoogleSpreadsheet = function () {
					...
					var c = this,
						...
						m = function () {
							...
							var a =
									("ABCDEFGHIJKLMNOPQRSTUVWXYZ".charAt(h.startColumn || 0) ||
										"A") +
									((h.startRow || 0) + 1),
								c =
									"ABCDEFGHIJKLMNOPQRSTUVWXYZ".charAt(F(h.endColumn, -1)) ||
									"ZZ";
							l(h.endRow) && (c += h.endRow + 1);
							return a + ":" + c;
						};
					d &&
						(delete h.googleSpreadsheetKey,
						a(function (a) {
							a = a.values;
				...
				b.prototype.parseColumn = function (a, c) {
					var b = this.rawColumns,
						...
						l = this.chartOptions,
						n,
						p = (this.options.columnTypes || [])[c];
					l =
						m &&
						((l && l.xAxis && "category" === L(l.xAxis)[0].type) ||
							"string" === p);
					for (b[c] || (b[c] = []); g--; ) {
						var r = f[g] || a[g];
						var q = this.trim(r);
						...
						l || (0 === g && e)
							...
							? ((a[g] = u),
							  31536e6 < u && "float" !== p
									? (a.isDatetime = !0)
									: (a.isNumeric = !0),
							  "undefined" !== typeof a[g + 1] && (n = u > a[g + 1]))
							: (q && q.length && (k = this.parseDate(r)),
							  m && A(k) && "float" !== p
									? ((f[g] = r),
									  (a[g] = k),
									  (a.isDatetime = !0),
					...
					if (m && n && this.options.sort)
						for (c = 0; c < d.length; c++)
							d[c].reverse(), e && d[c].unshift(d[c].pop());
				};
					...
					if (c) var e = c(a);
						...
						if (d)
							(c = this.dateFormats[d]) || (c = this.dateFormats["YYYY/mm/dd"]),
								(g = a.match(c.regex)) && (e = c.parser(g));
						else
						...
						g ||
							...
							"object" === typeof g && null !== g && g.getTime
								? (e = g.getTime() - 6e4 * g.getTimezoneOffset())
								: A(g) && (e = g - 6e4 * new Date(g).getTimezoneOffset()));
					}
					return e;
				};
				...
				b.prototype.complete = function () {
					...
					if (b.complete || b.afterComplete) {
						...
						0 < l.length &&
							0 < l[0].readers.length &&
							((f = a[l[0].readers[0].columnIndex]),
							"undefined" !== typeof f &&
								(f.isDatetime
									? (c = "datetime")
									: f.isNumeric || (c = "category")));
						if ("category" === c)
							for (d = 0; d < l.length; d++)
								for (f = l[d], k = 0; k < f.readers.length; k++)
									"x" === f.readers[k].configName &&
										(f.readers[k].configName = "name");
						for (d = 0; d < l.length; d++) {
							f = l[d];
							k = [];
					...
					a &&
						((a.afterComplete = function (a) {
							a &&
								(a.xAxis &&
									b.xAxis[0] &&
									a.xAxis.type === b.xAxis[0].options.type &&
									delete a.xAxis,
								b.update(a, c, !0));
						}),
						B(!0, b.options.data, a),
				...
				b.prototype.read = function (a, b) {
					...
					this.readers.forEach(function (e) {
						...
						c
							...
							? H.prototype.setNestedProperty(d, g, e.configName)
							: (d[e.configName] = g);
					});
	...
	p(
		...
		[b["Core/Globals.js"], b["Core/HttpUtilities.js"], b["Extensions/Data.js"]],
		function (b, e, p) {
			b.ajax = e.ajax;
			b.getJSON = e.getJSON;
			b.post = e.post;
